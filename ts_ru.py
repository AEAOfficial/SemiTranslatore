            ...
            with open(entry.path, encoding="utf8") as file:
                ...
                for line in file:
                    ...
                    if "displayText:" in line:
                        p = gtranslate.translate(CUT, "Russian")
                        fs = open('./tst/tst.ru.txt',"a+",encoding="utf-8")
                        tx = f'{p.result} \n'.replace(DT, 'displayText_ru:').replace('\ N', '\n').replace('\ n', '\\n')
                        fs.write(tx)
                        fs.close()
                    if "displayDescription:" in line:
                        p = gtranslate.translate(CUT, "Russian")
                        fs = open('./tst/tst.ru.txt',"a+",encoding="utf-8")
                        tx = f'{p.result} \n'.replace(DDs, 'displayDescription_ru:').replace('\ N', '\\n').replace('\ n', '\\n')
                        fs.write(tx)
                        fs.close()
