...
unit_stats = input(">>> PRESS ENTER TO GENERATE UNIT TS FILES \n") # INPUT STRINGS TO BE FOUND SEPERATED BY COMMAS
print('>>> Umm uh..')    
    ...
    for entry in scantree(sys.argv[1] if len(sys.argv) > 1 else '.'):
        ...
        if entry.is_file() and entry.name.endswith('.ini'):
            ...
            with open(entry.path, encoding="utf8") as file:
                ...
                for line in file:
                    DT = "DisplayText:"
                    DDs = "DisplayDescripción:"  
                    CUT = line
                    if "displayText:" in line:
                        p = gtranslate.translate(CUT, "Spanish")
                        fs = open('./tst/tst.es.txt',"a+",encoding="utf-8")
                        tx = f'{p.result} \n'.replace(DT, 'displayText_es:').replace('\ N', '\n').replace('\ n', '\\n')
                        fs.write(tx)
                        fs.close()
                    if "displayDescription:" in line:
                        p = gtranslate.translate(CUT, "Spanish")
                        fs = open('./tst/tst.es.txt',"a+",encoding="utf-8")
                        tx = f'{p.result} \n'.replace(DDs, 'displayDescription_es:').replace('\ N', '\\n').replace('\ n', '\\n')
                        fs.write(tx)
                        fs.close()
